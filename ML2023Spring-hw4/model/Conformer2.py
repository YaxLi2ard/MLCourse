...
class RelativeMultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        # QKV 和 位置嵌入线性变换
        kernel_size = 31
        self.query_proj = ConvProj(in_dim=d_model, kernel_size=kernel_size, bias=True)
        self.key_proj = ConvProj(in_dim=d_model, kernel_size=kernel_size, bias=True)
        self.value_proj = ConvProj(in_dim=d_model, kernel_size=kernel_size, bias=True)
        self.pos_proj = ConvProj(in_dim=d_model, kernel_size=kernel_size, bias=False)  # 位置编码不使用偏置
