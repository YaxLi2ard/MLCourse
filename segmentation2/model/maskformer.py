...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None):
        ...
        out = {
            'pred_logits': predictions_class[-1],  # 最后一层预测的类别 [b, num_q, num_classes+1]
            'pred_masks': predictions_mask[-1],  # 最后一层预测的mask [b, num_q, h, w]
...
if __name__ == '__main__':
    ...
    x = [torch.randn(1, 256, 160, 160), torch.randn(1, 256, 80, 80), torch.randn(1, 256, 40, 40), torch.randn(1, 256, 20, 20)]
    y = model(x[1:], x[0])
    for i in y.keys():
        print(i)
