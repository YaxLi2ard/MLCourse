...
class PoolingAttention(nn.Module):
    def __init__(self, dim, num_heads=2, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        # QK 缩放系数（如果没传入，就用标准 Transformer 的 1/sqrt(head_dim)）
    ...
    def forward(self, x, H, W, d_convs=None):
        B, N, C = x.shape
        # 生成q: [B, num_heads, N, head_dim]
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        pools = []
        x_ = x.permute(0, 2, 1).reshape(B, C, H, W)  # [B, C, H, W]
        for (pool_ratio, l) in zip(self.pool_ratios, d_convs):
            # 自适应平均池化到 (H/ratio, W/ratio)
            pool = F.adaptive_avg_pool2d(x_, (round(H / pool_ratio), round(W / pool_ratio)))
            pool = pool + l(pool)  # l是dw conv，l(pool)其实是用dw卷积生成 相对位置编码，再加到池化特征上
            pools.append(pool.view(B, C, -1))  # [B, C, N']
        # 在通道维拼接所有尺度的池化结果
        pools = torch.cat(pools, dim=2)  # [B, C, N']
        pools = self.norm(pools.permute(0, 2, 1))  # [B, N', C]
        # 生成 k/v: [2, B, num_heads, N', head_dim]
...
'''  Transformer Block 变体 区别是1.自注意力加了多尺度池化 2.ffn换成了mobilenet里的倒残差(dw卷积) '''
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, kernel_size=3, in_chans=3, embed_dim=768, overlap=True):
        super().__init__()
        # 把输入的 img_size / patch_size 转成 (高, 宽) 二元组
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)

        self.img_size = img_size
        self.patch_size = patch_size
        # 确保图像大小可以被 patch_size 整除
        assert img_size[0] % patch_size[0] == 0 and img_size[1] % patch_size[1] == 0, \
            f"img_size {img_size} should be divided by patch_size {patch_size}."
        # patch 后网格的高宽（H', W'）
        self.H, self.W = img_size[0] // patch_size[0], img_size[1] // patch_size[1]
        self.num_patches = self.H * self.W  # patch的数量
        if not overlap:
            self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
        else:  # kernel_size大于patch_size时，patch间会有重叠，stride=patch_size，padding=kernel_size // 2，下采样倍率还是patch_size
...
class PyramidPoolingTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 320, 512],
        ...
        # pyramid pooling ratios for each stage 每个stage的PoolingAttention中的多尺度下采样的比例
        ...
        # PoolingAttention 中，池化后的卷积，相当与 相对位置编码
        ...
        # transformer encoder
        # stage中每一层的drop path 概率，层数越深概率越大
...
if __name__ == '__main__':
    ...
    for yi in y:
        print(yi.shape)
    #  (1, 64, 80, 80) (1, 128, 40, 40) (1, 320, 20, 20) (1, 512, 10, 10)